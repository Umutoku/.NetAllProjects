...
{
    x.AddConsumer<OrderCreatedEventConsumer>(); // consumerımızı ekliyoruz
    x.AddConsumer<PaymentFailedEventConsumer>();
    x.UsingRabbitMq((context, cfg) =>
    {
        cfg.Host(builder.Configuration.GetConnectionString("RabbitMQ"));
        cfg.ReceiveEndpoint(RabbitMQSettings.StockOrderCreatedEventQueueName, e => // queue ismi
        {
            e.ConfigureConsumer<OrderCreatedEventConsumer>(context); // consumerımızı configure ediyoruz
        });
        cfg.ReceiveEndpoint(RabbitMQSettings.StockPaymentFailedEventQueueName, e => // queue ismi
        {
