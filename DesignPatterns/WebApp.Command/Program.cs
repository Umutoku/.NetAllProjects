...
builder.Services.AddSingleton(typeof(IConverter), new SynchronizedConverter(new PdfTools())); // sayesinde Pdf dönüştürme işlemleri gerçekleştirilebilir
...
if (!userManager.Users.Any())
{
    ...
    {
        appIdentityDbContext.Products.Add(new Product() { Name = $"Product {x}", Price = x * 10, Stock = x * 10 });
    });

    appIdentityDbContext.SaveChanges(); // Değişiklikler veritabanına kaydedilir
}
