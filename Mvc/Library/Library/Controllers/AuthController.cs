...
namespace Library.Controllers
{
    public class AuthController : Controller
    {
        ...
        //kullanıcının şifresi kriptolu veri ile eşleşiyor mu?
        //kullanıcının rolüne göre sayfa yönlendirmesi
        [HttpPost]
        public async Task<IActionResult> login(AppUser user)
        {
            if(_repoUser.Any(x=>x.UserName==user.UserName && x.Status!=DataStatus.Deleted))
            {
                AppUser selectedUser = _repoUser.Default(x=>x.UserName==user.UserName&& x.Status != DataStatus.Deleted);
